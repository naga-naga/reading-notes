...
- 列名を明示的に指定しないアンチパターン
- 問題
  - 列の追加・削除などが起こると、暗黙の列を使ったクエリが期待通り動かなくなる
  - 不要な列までフェッチすると、パフォーマンス的に無駄
- アンチパターンを使ってもよい場合
  - アドホックなクエリを書きたい場合
  - リスクを承知のうえで、開発の効率や可読性などを取りたい場合
- 解決策
  - 列を明示的に指定する
    - `SELECT` したときに列の順番が変わらない
    - 列が増えてもクエリの結果に影響がない
    - `INSERT` する際に列が消えていたらエラーを吐いてくれる

# 第19章 リーダブルパスワード
- パスワードを平文や復号可能な暗号化を施して保存するアンチパターン
- 問題
  - 重大なセキュリティリスクである
  - SQL 文は傍受できる
  - 電子メールも傍受できる
- アンチパターンを使ってもよい場合
  - アプリケーション自体が別のサービスのクライアントとなる場合
    - 当然アプリケーション内で復号可能でなければならない
  - 悪用する人がいないと確信できるようなアプリケーションの場合
    - 身内で使うものとか
    - 大した機密情報のないものとか
- 解決策
  - ソルトを付けてパスワードハッシュを格納する
    - ソルトを付けることでレインボーテーブルにも対応できる
  - SQL クエリの中でもハッシュ化する
    - SQL クエリが傍受される可能性があるため
  - ブラウザとアプリケーションの間は HTTPS を使う
    - ここだけはどうしてもパスワードをそのまま送る必要がある
  - パスワードを再設定したい場合はリセットする
    - 一時パスワードをメールで送ったり
    - トークン付きのリンクを送ったり
...
- 問題
  - 最小の欠番を探す処理が無駄
  - 最小の欠番を探す処理を並列実行すると ID が被る可能性がある
  - 既存の行の ID を振り直すのも無駄
    - 欠番以降の全ての行を振り直す必要があり、また、行が増えると自動採番によりまたずれる
- アンチパターンを使ってもよい場合
  - 疑似キーの値の変更を正当化する理由はない
  - 自然キーなら変わりうる
- 解決策
  - 疑似キーの欠番は埋めない
  - UUID を使うのもよい？
    - タイプしづらい、値からパターンを推測しづらい、整数よりスペースが必要などといったデメリットもある

# 第22章 シー・ノー・エビル
- エラーハンドリングをしなかったりデバッグ時に実行されたクエリを見ないアンチパターン？
- 問題
  - エラーハンドリングをしないとエンドユーザにはブランクページや謎の例外メッセージが表示されてしまう
  - デバッグ時も最終的に組み立てられたクエリを見ないと時間やエネルギーを浪費する可能性がある
- アンチパターンを使ってもよい場合
  - アプリケーション終了時の `close` など？
    - きっとクリーンアップされる
  - 自身に責任のない例外を呼び出し側まで通過させる場合
- 解決策
  - エラーハンドリングする
  - 構築された SQL クエリを確認できるようにしておく
    - クエリを一時変数に受ける
    - SQL をログなどに出力する

# 第23章 ディプロマティック・イミュニティ
- SQL のみを特別扱いしてバージョン管理や自動テスト、ドキュメントの作成などのベストプラクティスに沿わないアンチパターン
- 問題
  - 次第に保守できなくなり、プロジェクトは失敗に向かう
- アンチパターンを使ってもよい場合
  - 一度限りのテストやクエリを書く場合
  - この場合、使い終わったコードはすぐに削除する
- 解決策
  - 品質問題に包括的に取り組む
    - 文書化
      - ER 図やテーブルの説明など
      - データベースの文書化には価値があるらしい
    - バージョン管理
      - スキーマ定義のファイルや、初期状態を構築するためのデータ、各種スクリプトなどを含める
    - テスティング
      - 存在すべきテーブルやビューが存在するか、また、存在すべきでないものが存在しないか、など
    - 複数のブランチを扱う
      - 開発環境、ステージング環境、テスト環境など、環境ごとにデータベースを用意する

# 第24章 マジックビーンズ
- 全てのモデルクラスがアクティブレコードの基底クラスを継承するアンチパターン？
- 問題
  - モデルがデータベーススキーマに強く依存してしまう
    - データベースをリファクタリングする際に、モデルだけでなくそれを利用するアプリケーションのコードも書き換える必要がある
    - Rails はどうやってこの辺りを上手くやっているのだろうか？
      - 動的なメソッド生成とかかしら
  - CRUD 機能が公開されてしまう
    - アクティブレコードの基底クラスから派生したモデルクラスが、基底クラスの CRUD を公開するのは果たして良いことか？
  - ドメインモデル貧血症をもたらす
    - モデルをシンプルなデータアクセスオブジェクトとして扱うと、モデルの外部にビジネスロジックが書かれ、凝集度が下がる
  - ユニットテストが困難
    - モデル自身の振舞いのテストをデータベースアクセスから分離できない
    - コントローラのテストも複雑になる
- アンチパターンを使ってもよい場合
  - シンプルなデータアクセスオブジェクトで十分な場合
  - プロトタイプ作成など、開発速度が求められる場合
    - リファクタリングの時間も取ること
- 解決策
  - モデルがアクティブレコードを持つようにする
    - is-a ではなく has-a の関係にする
    - データアクセスオブジェクトを分離すると、ユニットテストもやりやすくなる

# 第25章 砂の城
- 想定が足りていないというアンチパターン
- 問題
  - どのような問題が起こるか、それぞれの事象に対してどのように対応するかといった想定が不足していると、サービスの安定稼働が難しい
- アンチパターンを使ってもよい場合
  - コストとの兼ね合いでどこまでやるか、という話
  - どこまでコストをかけて安定稼働させるか、あるいは、どの程度の停止時間なら許容できるかを考えておく
- 解決策
  - どのようなトラプルが発生するかを可能な限り把握しておくこと
  - 対策例
    - ベンチマーク
      - どの程度まで処理が可能なのかを計測しておく
      - 現実に即したシナリオ、データで行うこと
    - テスト環境の構築
      - 本番環境と同じ環境をもう1セット用意する
    - 例外処理
      - どのような例外が発生しうるかを把握する
    - バックアップ
      - 最後の砦
      - RAID やレプリケーションはバックアップにならない
        - ミスが即座に伝播してはバックアップとしての意味がない
    - 高可用性
      - フェイルオーバーの仕組みを構築しておくなど
    - ディザスタリカバリ
      - 自然災害などへの対策
      - 物理的に離れた別のデータセンターを使うなど
    - 運用ポリシーの作成
      - 自動復旧できない場合の対応？

# 付録 正規化のルール
- 第1正規形
  - 同じ定義息の複数の値が複数の列に格納されていない
    - マルチカラムアトリビュートになっていない
  - 複数の値が単一の列に格納されていない
    - ジェイウォークになっていない
  - 多対多の関連は別テーブルに切り出す
- 第2正規形
  - 主キーの一部に従属する列は別テーブルに切り出す
- 第3正規形
  - 非キー同士の従属関係は別テーブルに切り出す
- ボイスコッド正規形
  - 主キー同士でも従属関係があれば別テーブルに切り出す
- 第4正規形
  - 1つのテーブルで複数の多対多関係を表している場合は、それぞれ別テーブルに切り出す
- 第5正規形
  - （正直あまり分かっていない）
- ドメインキー正規形
  - （正直あまり分かっていない）
- 第6正規形
  - （正直あまり分かっていない）
  - 属性の変更履歴のサポートに使われるらしい？
  - データウェアハウスの一部などで使われているらしい
- 感想
  - 普段あまり正規化については意識していないが、それでも大体いい感じになっている気がするのは、モデルから作るからかもしれないなと思った
